...
static int _get_packet(unsigned char *packet, int *len)
{
    ...
    if (*len < 48) {
        MOLMC_LOGE("epoch_time", "packet buf too short!\n");
        return -1;
    }
...
#ifdef NTP_DEBUG
    MOLMC_LOGD("epoch_time", "LI=%d  VN=%d  Mode=%d  Stratum=%d  Poll=%d  Precision=%d\n",
              ntp_packet.li, ntp_packet.vn, ntp_packet.mode,
              ntp_packet.stratum, ntp_packet.poll, ntp_packet.prec);
    MOLMC_LOGD("epoch_time", "Delay=%.1f  Dispersion=%.1f  Refid=%u.%u.%u.%u\n",
...
static uint64_t _get_timestamp_from_ntp(const char *host)
{
    ...
    if (fd < 0) {
        MOLMC_LOGE("epoch_time", "udp create error!");
        return 0;
    }

    ret = _get_packet(write_buf, &write_len);
    if (ret < 0) {
        MOLMC_LOGE("epoch_time", "get_packet error!");
        goto do_exit;
    }

    ret = HAL_UDP_write((void *)fd, write_buf, write_len);
    if (ret < 0) {
        MOLMC_LOGE("epoch_time", "udp write error!");
        goto do_exit;
    }

    ret = HAL_UDP_readTimeout((void *)fd, read_buf, sizeof(read_buf), 3000);
    if (ret < 0) {
        MOLMC_LOGE("epoch_time", "udp read error!");
        goto do_exit;
    }
